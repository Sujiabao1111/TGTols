...
func (c *HedoClient) LaunchGame(userid int, gameCode, apibase string) (string, error) {
	...
	var resp APIResponse
	// 假设接口是 /launch
	_, err := c.client.R().SetBody(payload).SetResult(&resp).Post(apibase + "Game/OpenGameByCode")
	if err != nil {
		return "", err
	}
	if resp.Code != 200 {
...
func (c *HedoClient) OpenGame(loginId, password string, providerId int, gameCode string, isMobile bool, lang, apibase, agentid string) (string, error) {
	...
	// 接口路径为 /Game/OpenGame
	res, err := c.client.R().SetBody(req).SetResult(&resp).Post(apibase + "Game/OpenGameByCode")
...
func (c *HedoClient) GetPlayerDailySummary(loginId string, dateStr string, agentid, agentapi string) (*PlayerSummaryData, error) {
	// 1. 构造时间范围 (全天 00:00:00 - 23:59:59)
	// 假设服务器时区和三方一致 (东八区)，如果不一致需要调整 time.Location
	loc, _ := time.LoadLocation("Asia/Shanghai")
	t, _ := time.ParseInLocation("2006-01-02", dateStr, loc)

	fromDate := t.Format(time.RFC3339) // 2023-10-27T00:00:00+08:00
	toDate := t.Add(24 * time.Hour).Add(-1 * time.Millisecond).Format(time.RFC3339)

	req := PlayerSummaryRequest{
		AgentId:          agentid,
		LoginId:          loginId,
		SummaryDate:      1, // Daily
		GameProviderCode: 0, // All Providers
		FromDate:         fromDate,
		ToDate:           toDate,
	}
	...
	// 假设接口路径
	res, err := c.client.R().SetBody(req).SetResult(&resp).Post(agentapi + "/Report/TransactionSummaryByPlayer")

	if err != nil {
		return nil, fmt.Errorf("network error: %v", err)
	}
	if res.StatusCode() != 200 || resp.Code != 0 { // 假设 Code 0 是成功
		return nil, fmt.Errorf("provider error: %s", resp.Message)
	}
